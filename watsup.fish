function w
    ...
    set action (printf "do-nothing\nstart\nstart-edit\nstop\nstop-edit\nabort\nedit\nedit-last\nchange\nhelp" | fzf --height=12 --prompt="Select an action: ")
    switch $action
        case 'start*'
        ...
        case edit
            set frame_line ($WATSUP_BINARY log --from 2000-01-01 | grep -E '^\s+[0-9a-f]+' | tail -20 | tac | fzf --prompt="Select a frame to edit: ")
        ...
        case help
            ...
            echo ""
            echo "Actions:"
            echo "  start / start-edit       Start tracking a new project"
            echo "  stop / stop-edit         Stop current tracking"
            echo "  abort                    Cancel current tracking without saving"
            echo "  edit                     Edit a past frame"
            echo "  edit-last                Edit the currently in-progress recording (if any) otherwise the last recording"
            echo "  change                   Stop current tracking and start tracking a different project"
            echo ""
